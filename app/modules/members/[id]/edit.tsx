...
export default function EditMemberScreen() {
  ...
  const loadMemberData = async () => {
    ...
    try {
      ...
      const memberData = await getMemberById(id as string);
      console.log('Member data loaded:', memberData);
      
      setMemberName(memberData.memberName || '');
      setEmail(memberData.email || '');
      setMobile1(memberData.mobile1 || '');
      setMobile2(memberData.mobile2 || '');
      setOrganizationName(memberData.organizationName || '');
      setActive(memberData.active !== undefined ? memberData.active : true);
    } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await updateMember(id as string, payload);
      Toast.show({
        type: 'success',
        text1: 'Success',
  ...
  if (loadingData) {
    return (
      <ThemedView style={[styles.container, { backgroundColor }]}>
        <NavigationHeader title="Edit Member" backPath="/modules/members" />
