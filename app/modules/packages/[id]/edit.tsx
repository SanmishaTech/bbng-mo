...
export default function EditPackageScreen() {
  ...
  const loadPackageData = async () => {
    ...
    try {
      ...
      setPeriodMonths(Number(packageData?.periodMonths) || 1);
      setIsVenueFee(Boolean(packageData?.isVenueFee));
      setSelectedChapterId(packageData?.chapterId ? Number(packageData.chapterId) : null);
      setBasicFees(String(packageData?.basicFees || ''));
      setGstRate(String(packageData?.gstRate || '18'));
      setActive(Boolean(packageData?.active));
    } catch (error) {
      console.error('Error loading package:', error);
      Toast.show({
        type: 'error',
        text1: 'Error',
        text2: 'Failed to load package data',
      });
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      await updatePackage(id as string, payload);
      Toast.show({
        type: 'success',
        text1: 'Success',
  ...
  if (loading) {
    return (
      <ThemedView style={[styles.container, { backgroundColor }]}>
        <NavigationHeader title="Edit Package" backPath="/modules/packages" />
          ...
          <View style={[styles.formCard, { backgroundColor: colors.card }]}>
            ...
            {isVenueFee && (
              <View style={styles.fieldGroup}>
                ...
                {loadingChapters ? (
                  <View style={[styles.chaptersLoadingContainer, { backgroundColor: colors.surface }]}>
                    <ActivityIndicator size="small" color={colors.primary} />
                    <Text style={[styles.chaptersLoadingText, { color: colors.text }]}>
                      Loading chapters...
                    </Text>
                  </View>
                ) : (
