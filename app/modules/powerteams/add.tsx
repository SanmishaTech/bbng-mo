...
import { createPowerTeam, getAllCategories, getSubCategoriesByCategoryId, Category, SubCategorySummary } from '@/services/powerTeamService';
import Checkbox from 'expo-checkbox';

export default function AddPowerTeamScreen() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await createPowerTeam(formData);

      Toast.show({
        type: 'success',
        text1: 'Success',
        text2: 'Power team created successfully',
      });
      ...
      Toast.show({
        type: 'error',
        text1: 'Error',
        text2: error?.message || 'Failed to create power team',
      });
    } finally {
      setLoading(false);
    }
  ...
  if (loadingData) {
    return (
      <ThemedView style={[styles.container, { backgroundColor }]}>
        <NavigationHeader title="Add Power Team" backPath="/modules/powerteams" />
      ...
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent}>
        ...
        >
          {loading ? (
            ...
            <Text style={styles.submitButtonText}>Create Power Team</Text>
          )}
        </TouchableOpacity>
      </ScrollView>
    </ThemedView>
