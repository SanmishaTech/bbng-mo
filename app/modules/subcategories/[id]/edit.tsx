...
export default function EditSubCategoryScreen() {
  ...
  const loadSubCategoryData = async () => {
    setLoading(true);
    try {
      const subcategoryData = await getSubCategoryById(id as string);
      setSubcategoryName(subcategoryData?.name || '');
      // Ensure categoryId is stored as a number
      setSelectedCategoryId(subcategoryData?.categoryId ? Number(subcategoryData.categoryId) : null);
    } catch (error) {
      console.error('Error loading subcategory:', error);
      Toast.show({
        type: 'error',
        text1: 'Error',
        text2: 'Failed to load sub-category data',
      });
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      await updateSubCategory(id as string, payload);
      Toast.show({
        type: 'success',
        text1: 'Success',
      ...
      Toast.show({
        type: 'error',
        text1: 'Error',
        text2: 'Failed to update sub-category',
      });
    } finally {
      setSubmitting(false);
    }
  ...
  if (loading) {
    return (
      <ThemedView style={[styles.container, { backgroundColor }]}>
        <NavigationHeader title="Edit Sub-Category" backPath="/modules/subcategories" />
        <View style={styles.loadingContainer}>
          <ActivityIndicator size="large" color={colors.primary} />
          <Text style={[styles.loadingText, { color: colors.text }]}>
            Loading sub-category data...
          </Text>
        </View>
      </ThemedView>
    );
        ...
        >
          <View style={[styles.formCard, { backgroundColor: colors.card }]}>
            ...
            <View style={styles.fieldGroup}>
              ...
              {loadingCategories ? (
                <View style={[styles.categoriesLoadingContainer, { backgroundColor: colors.surface }]}>
                  <ActivityIndicator size="small" color={colors.primary} />
                  <Text style={[styles.categoriesLoadingText, { color: colors.text }]}>
                    Loading categories...
                  </Text>
                </View>
              ) : (
