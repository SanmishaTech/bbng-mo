...
import { getSubCategories, deleteSubCategory, SubCategory } from '@/services/subCategoryService';
...
export default function SubCategoriesScreen() {
  ...
  // Fetch subcategories
  const fetchSubcategories = async (page: number = currentPage, refresh: boolean = false) => {
    try {
      if (refresh) {
        setRefreshing(true);
      } else {
      ...
      const response = await getSubCategories(page, recordsPerPage, search, sortBy, sortOrder);
      ...
      Alert.alert('Error', error?.message || 'Failed to fetch subcategories');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const renderSubcategory = ({ item }: { item: SubCategory }) => (
    <View style={[styles.subcategoryCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
      ...
      <View style={styles.subcategoryInfo}>
        ...
        {item.categoryName && (
          <View style={styles.infoRow}>
            <IconSymbol name="folder.fill" size={14} color={colors.placeholder} />
            <Text style={[styles.infoText, { color: colors.placeholder }]}>
  ...
  const renderEmpty = () => (
    <View style={styles.emptyState}>
      ...
      <Text style={[styles.emptyStateTitle, { color: colors.text }]}>No Subcategories Found</Text>
      <Text style={[styles.emptyStateSubtitle, { color: colors.placeholder }]}>
        {search ? 'Try adjusting your search' : 'Tap the + button to add your first subcategory'}
      </Text>
    </View>
  );
    ...
    return (
      <View style={styles.paginationContainer}>
        <TouchableOpacity
          ...
          onPress={() => currentPage > 1 && fetchSubcategories(currentPage - 1)}
        ...
        <TouchableOpacity
          ...
          onPress={() => currentPage < totalPages && fetchSubcategories(currentPage + 1)}
      ...
      >
        <View style={styles.headerContent}>
          ...
          <View style={styles.headerTextContainer}>
            ...
            <Text style={styles.headerSubtitle}>{totalSubcategories} total subcategories</Text>
          </View>
          <View style={{ width: 40 }} />
        </View>
      </LinearGradient>
        ...
        <FlatList
          ...
          refreshControl={
            <RefreshControl
              ...
              onRefresh={() => fetchSubcategories(currentPage, true)}
