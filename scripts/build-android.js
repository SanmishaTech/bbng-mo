...
/**
 * Build Android App using EAS Build API
 * 
 * This script triggers an Android build using the Expo Application Services (EAS) Build API.
 * It can be used for CI/CD pipelines or manual build triggers.
 * 
 * Usage:
 *   node scripts/build-android.js [profile] [--wait]
 * 
 * Examples:
 *   node scripts/build-android.js preview
 *   node scripts/build-android.js production --wait
 *   EAS_TOKEN=your_token node scripts/build-android.js preview
 */
...
async function triggerBuild(token) {
  console.log(`🚀 Triggering Android build with profile: ${BUILD_PROFILE}`);
  ...
  while (attempts < maxAttempts) {
    ...
    if (build.status === 'finished') {
      ...
      if (build.artifacts && build.artifacts.buildUrl) {
        console.log(`📥 Download APK/AAB: ${build.artifacts.buildUrl}`);
      }
      return build;
...
// Main execution
async function main() {
  console.log('🏗️  EAS Android Build Script');
